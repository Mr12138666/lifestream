package com.sunrisejay.framework.common.util;
...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checklifestreamId(String lifestreamId) {
        // 检查长度
        if (lifestreamId.length() < ID_MIN_LENGTH || lifestreamId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(lifestreamId).matches();
    }
